CannabisCoin-qt: Qt4 GUI for CannabisCoin
===============================
...
Alternatively, install Qt Creator and open the `CannabisCoin-qt.pro` file.

An executable named `CannabisCoin-qt` will be built.
...
A warning for people using the *static binary* version of CannabisCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of CannabisCoin is linked against libdb5.3.
...
installed causes CannabisCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
