...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("CannabisCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CannabisCoinTEST2\r");
            Send(hSocket, "WHO #CannabisCoinTEST2\r");
        } else {
            // randomly join #CannabisCoin00-#CannabisCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #CannabisCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CannabisCoin%02d\r", channel_number).c_str());
        }
